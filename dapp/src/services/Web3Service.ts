...
export async function getTopic(title: string): Promise<Topic> {
    ...
    return {
        ...
        category: Number(topic.category), // Convert BigInt to number
        amount: topic.amount,
        responsible: topic.responsible,
        status: Number(topic.status), // Convert BigInt to number
        createdDate: Number(topic.createdDate), // Convert BigInt to number
        startDate: Number(topic.startDate), // Convert BigInt to number
        endDate: Number(topic.endDate), // Convert BigInt to number
    } as Topic;
}
...
export async function getTopics(page: number = 1, pageSize: number = 10): Promise<TopicPage> {
    ...
    const topics = result.topics.filter(t => t.title).map(topic => ({
        title: topic.title,
        description: topic.description,
        category: Number(topic.category), // Convert BigInt to number
        amount: topic.amount,
...
export async function getQuota(): Promise<ethers.BigNumberish>{
    const contract = getContract();
    return contract.getQuota() as Promise<ethers.BigNumberish>;
}
