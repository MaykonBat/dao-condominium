...
describe("Condominium", function () {
  ...
  it("Should NOT add resident (duplicated)", async function () {
    const { contract, manager, accounts } = await loadFixture(deployFixture);
    await contract.addResident(accounts[1].address, 2102);
  ...
  it("Should NOT add resident (residence)", async function () {
    ...
    await expect(contract.addResident(accounts[1].address, 21020)).to.be.revertedWith("This residence does not exists");
  });
  ...
  it("Should NOT add counselor (exists)", async function () {
    ...
    await expect(contract.setCounselor(accounts[2].address, false))
      .to.be.revertedWith("Counselor not found");
  });
  ...
  it("Should NOT add topic (permission)", async function () {
    ...
    const instance = contract.connect(accounts[1]);
    await expect(instance.addTopic("topic 1", "description 1", Category.DECISION, 0, manager.address))
  ...
  it("Should NOT remove topic (permission)", async function () {
    ...
    await expect(intance.removeTopic("topic 1")).to.be.revertedWith("Only the manager can do this");
  });
  ...
  it("Should NOT vote (Topic exists)", async function () {
    const { contract, manager, accounts } = await loadFixture(deployFixture);
  ...
  it("Should NOT vote (Resident Exists)", async function () {
    const { contract, manager, accounts } = await loadFixture(deployFixture);
    ...
    await expect(contract.vote("topic 1", Options.YES)).to.be.revertedWith("Resident not found");
  });
  ...
  it("Should NOT transfer (funds)", async function () {
    ...
    await expect(contract.transfer("topic 1", 100)).to.be.revertedWith("Insufficient funds");
  });
